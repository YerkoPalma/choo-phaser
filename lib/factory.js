...
var storage = require('lowdb-gh-adapter')
...
function GameFactory (opts, state, emit) {
  ...
  if (process.env.NODE_ENV === 'production') {
    this._adapter = storage({
      file: this._state.student + '.json',
      repo: 'kids',
      user: 'YerkoPalma',
      name: 'YerkoPalma',
...
GameFactory.prototype.getState = function () {
  ...
  var create = () => {
    ...
    this._sprites.map(sprite => {
      if (sprite.isOption) {
        // add sprite
        var imgOption = this._game.add.sprite(0, 0, sprite.name)
        // hide option for now
        imgOption.scale.setTo(0)
        imgOption.position.x = (this.imgOptions.length * 150) + this._game.world.centerX / 2
        imgOption.position.y = 50
        imgOption.inputEnabled = true
        imgOption.input.useHandCursor = true
        imgOption.events.onInputUp.add(this.onAnswer(imgOption), imgOption)
        this.imgOptions.push(imgOption)
      }
    })
...
GameFactory.prototype.writeInstructions = function (line, wordIndex, lineIndex, start, lineDelay = 400, wordDelay = 60) {
  ...
  if (lineIndex === this._state.steps[this._state.currentStage][this._state.currentStep].instructions.length) {
    ...
    if (this._state.steps[this._state.currentStage][this._state.currentStep].images) {
      this.imgOptions.map((opt, i) => {
        opt.scale.setTo(1)
        this._emit('log:info', opt.width)
        opt.scale.setTo(120 / opt.width)
      })
    } else {
      this.options.map((opt, i) => {
...
GameFactory.prototype.onStageFinished = function () {
  ...
  if (this._state.currentStage < this._state.steps.length - 1) {
    ...
    this._sprites.forEach(_sprite => {
      if (!_sprite.isOption) this.sprites[_sprite.name].destroy()
    })
    ...
    this._game.add.tween(this.main).to({
      ...
      // save results
      this._state.stages[this._id] = this.results
      // this.results = []
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        .then(db => {
          // var data = {}
          // TODO: assign a valid date
          var today = new Date()
          var key = today.getFullYear().toString() + today.getMonth().toString() + today.getDate().toString()
          // data[key] = this._state.stages
          db.defaults({})
            .set(key + '.' + this._id, this.results)
            .write()
            .then(() => this._emit(this._state.events.PUSHSTATE, '/'))
            .catch(err => {
              throw err
            })
        })
...
GameFactory.prototype.onAnswer = function (opt) {
  return () => {
    if (this._state.steps[this._state.currentStage][this._state.currentStep].images) {
      if (this._state.steps[this._state.currentStage][this._state.currentStep].correctOption === opt.key) {
