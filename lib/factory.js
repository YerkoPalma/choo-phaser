function GameFactory (opts, state, emit) {
  ...
  this.steps[0].forEach((step, i) => {
    this.steps[0][i] = Object.assign(step, { next: this.onNextStep })
  })
  this.background = {}
...
GameFactory.prototype.getState = function () {
  ...
  var create = () => {
    ...
    var mainAnim = this._main.animations[this._main.animations.main]
    this.main.animations.play(mainAnim.name, mainAnim.to, mainAnim.loop)
    this.writeInstructions([], 0, 0, new Date())
  }
  var update = () => {}
  return {
    preload,
  ...
  return function () {
    ...
    if (wordIndex === line.length) {
      ...
      setTimeout(() => {
        ...
        this._game.time.events.add(lineDelay, () => this.writeInstructions(line, wordIndex, lineIndex, new Date()), this)
      }, 1000)
    }
  }
}
GameFactory.prototype.onStageFinished = function (start) {
  var end = new Date()
  var results = {
    time: Math.abs((start.getTime() - end.getTime()) / 1000),
    resolved: true,
    mistakes: this.results[this._tate.currentStage].mistakes
  }
  this.results[this._state.currentStage] = results
  this.results[this._state.currentStage].mistakes = 0
  if (this._state.currentStage < this.steps.length - 1) {
...
GameFactory.prototype.onNextStep = function () {
  if (this._state.currentStep < this.steps[this._state.currentStage].length - 1) {
    var line = []
    var wordIndex = 0
    var lineIndex = 0
